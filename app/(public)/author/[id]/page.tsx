...
export async function generateMetadata({ params }: AuthorPageProps): Promise<Metadata> {
    ...
    if (!author) {
        return {
            title: "Author Not Found | Pain2Purpose",
        };
    }

    return {
        title: `${author.name} | Pain2Purpose`,
        description: `Explore insights and wisdom from ${author.name} — Pain2Purpose Counselling Practice.`,
        openGraph: {
            title: `${author.name} | Pain2Purpose`,
...
export default async function AuthorProfilePage({ params }: AuthorPageProps) {
    ...
    if (!author) {
        return (
            <div className="min-h-screen bg-p2p-cream flex items-center justify-center">
                <div className="text-center">
                    <h1 className="text-4xl font-serif font-bold text-p2p-charcoal mb-4">Author Not Found</h1>
                    <Link href="/" className="text-p2p-sage font-bold hover:underline">Return to Home</Link>
                </div>
            </div>
        );
    }
    ...
    const jsonLd = {
        ...
        "mainEntity": {
            "@type": "Person",
            "name": author.name,
            "description": `Founder & Counsellor at Pain2Purpose`,
            "image": author.profileImage,
            "url": `https://counsellingp2p.com/author/${id}`,
        }
    };
